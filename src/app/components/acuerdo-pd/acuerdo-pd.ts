...
type TipoEstructura = 'patrocinio' | 'individual';
type TipoArchivoDescarga = 'word' | 'excel';
...
export class AcuerdoPD {
  ...
  tipoEstructura = signal<TipoEstructura>('patrocinio');

  estructuraPatrocinio = signal<string[]>([
    'Codigo',
    'NombresC',
    'Cedula1',
    'Carrera1',
  ...
  estructuraIndividual = signal<string[]>([
    'Codigo',
    'NombresC',
    'Cedula1',
    'Carrera1',
  ...
  columnasEstructuraActual = computed(() =>
    this.tipoEstructura() === 'patrocinio'
      ? this.estructuraPatrocinio()
      : this.estructuraIndividual()
  );

  tituloEstructuraActual = computed(() =>
    this.tipoEstructura() === 'patrocinio'
      ? 'Estructura Patrocinio'
      : 'Estructura Plan Individual'
  );

  cambiarEstructura(tipo: TipoEstructura): void {
    this.tipoEstructura.set(tipo);
  }

  async descargarWordActual(): Promise<void> {
    await this.descargarDocumentoBase(this.tipoEstructura(), 'word');
  }

  async descargarExcelActual(): Promise<void> {
    await this.descargarDocumentoBase(this.tipoEstructura(), 'excel');
  }

  async descargarDocumentoBase(
    tipo: TipoEstructura,
    archivo: TipoArchivoDescarga
  ): Promise<void> {
    try {
      const ruta = this.obtenerRutaDocumento(tipo, archivo);
      const nombreArchivo = this.obtenerNombreDescarga(tipo, archivo);

      const response = await fetch(ruta);
      if (!response.ok) {
        throw new Error(`No se pudo descargar el archivo desde ${ruta}`);
      }

      const blob = await response.blob();
      saveAs(blob, nombreArchivo);
    } catch (error) {
      console.error('Error al descargar documento base:', error);
      alert('No se pudo descargar el archivo. Verifica que exista en assets/docs o assets/xlsx.');
    }
  }

  obtenerRutaDocumento(tipo: TipoEstructura, archivo: TipoArchivoDescarga): string {
    if (archivo === 'word') {
      return tipo === 'patrocinio'
        ? '/src/assets/docs/Patrocinio.docx'
        : '/src/assets/docs/individual.docx';
    }

    return tipo === 'patrocinio'
      ? '/src/assets/xlsx/Patrocinio.xlsx'
      : '/src/assets/xlsx/individual.xlsx';
  }

  obtenerNombreDescarga(tipo: TipoEstructura, archivo: TipoArchivoDescarga): string {
    if (archivo === 'word') {
      return tipo === 'patrocinio' ? 'Patrocinio.docx' : 'individual.docx';
    }

    return tipo === 'patrocinio' ? 'Patrocinio.xlsx' : 'individual.xlsx';
  }
        ...
        if (rows.length > 0) {
          ...
          if (primeraFila['Capacitaciones']) {
            console.log(
              ...
              primeraFila['Capacitaciones'],
              typeof primeraFila['Capacitaciones']
            );
          }
...
Fechas: ${fechas.length}`);
        ...
        const minLengthLog = Math.min(nombres.length, horas.length, fechas.length);
        console.log(`Usando longitud mínima: ${minLengthLog}`);
      }
  ...
  exportarExcel(): void {
    ...
    for (const persona of datos) {
      ...
      const nombreArchivoWord = `${persona['Codigo']}-${nombreLimpio}`;
